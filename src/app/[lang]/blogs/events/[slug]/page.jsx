...
export async function generateMetadata({ params }) {
  try {
    const blog = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/events?filters[slug][$eq]=${params.slug}&populate=image&populate=SEO`,
      {
        headers: {
...
export async function getData() {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/events?populate=*`,
      {
        next: { revalidate: 40 },
        headers: {
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_BEARER_TOKEN}`,
...
export default async function BlogDetail({ params: { slug } }) {
  ...
  return (
    <div className="pt-[108px] md:pt-[128px]">
      {blog.length > 0 ? (
        <>
          <div className="relative h-[70vh] w-full">
            <Image
              src={blog[0].attributes.images.data[0].attributes.url}
              fill
              sizes="100vw"
          ...
          <div className="max-w-[1500px] mx-auto px-6 md:px-9 ">
            <div className="relative grid grid-cols-1 gap-6 md:gap-12 md:grid-cols-3  my-10 ">
              <div className="grid col-span-1 md:col-span-2">
                ...
                <DescriptionComponent bio={blog[0].attributes.details} />
              </div>
